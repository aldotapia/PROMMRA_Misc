...
for (ntree in c(56, 128, 256, 512, 1028)){
  set.seed(123)
  model <- train(class~.,
                 data = for_training,
                 method = 'rf',
...
lapply(modellist, function(x) max(x$results$Kappa))

model <- modellist$`1028`
...
write.csv(x = model_sum,
          file = '~/Documents/1_dtw/results/rf_sum.csv',
          row.names = F)
...
for(i in seq_along(spls)){
  ...
  fname <- paste0('~/Documents/1_dtw/scenes/rf/rf_',
                  fnames[i])
